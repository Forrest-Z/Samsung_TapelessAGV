...
KuILBPFLocalizerPr::KuILBPFLocalizerPr()
{
	...
	m_IplMapImage = cvCreateImage(cvSize(1000,1000),8,3);//Result image initialization
	m_nMapSizeX=1000;
	m_nMapSizeY=1000;

	m_dHeight = 3.1;//1.74;//1.45;//1.74;
	m_bMapping=false;
}
...
void KuILBPFLocalizerPr::setRangeSensorParameter()
{
	...
	double dInterval = 1;
	double dRangeSensorOffset = KuRobotParameter::getInstance()->getURG04LXLaserXOffset();
	...
	dHeight=KuRobotParameter::getInstance()->getEllipseHeight();
	dWidth=KuRobotParameter::getInstance()->getEllipseWidth();
...
void KuILBPFLocalizerPr::start(int nPeriod,bool bMapping)
{
	if(false==m_bIsThreadFuncGenerated){
		...
		m_KuThread.start(doThread,this, 100);	
	}
}
...
of range sensor
*/
void KuILBPFLocalizerPr::calLampHeight(KuPose RobotPos, vector<FRegionData>* FRD)
{
...
of range sensor
*/
void KuILBPFLocalizerPr::registerLamp( KuPose RobotPos, vector<FRegionData> *FRD)
{
	double dy=0.0 ,dx=0.0;
	...
	{		
		...
		for(int j=0; j<i;j++)
		{
			...
			{			
				...
				if((*FRD)[i].th>150&&(*FRD)[i].th<30)
				{
					(*FRD)[i].th+=180;
				}
				else if((*FRD)[i].th<30&&(*FRD)[i].th>150)
				{
					(*FRD)[i].th-=180;
				}
...
void KuILBPFLocalizerPr::estimateRobotPosUsingParticleFilter()
{
	...
	if(m_bMapping==true)
	{
		...
		vector<FRegionData> FRD = m_FDetectorAlg.detectRegion(m_cvMatImageBright,true);	
		nLampNUM=FRD.size();
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			||fabs(RobotPos.getThetaDeg()-m_PreRobotpos.getThetaDeg())>10)
		{
			...
			vector<FTampletData> FTData = m_FDetectorAlg.checktempletRegion(m_cvMatImageT,true);

			vector<FTampletData> tempFTD ;
			vector<FTampletData> mergeFTD ;
			FTampletData observedFTD;

			for(int i=0; i<FTData.size();i++)
			{
				FTData[i].Rx=RobotPos.getXm();
				FTData[i].Ry=RobotPos.getYm();
				FTData[i].Rt=RobotPos.getThetaRad();
				FTData[i].th=RobotPos.getThetaRad();
				FTData[i].nmatchnum=1;
				FTData[i].similarity=0;
				FTData[i].ceilingheight=m_dHeight;
				FTData[i].add=true;
			}
			...
			if(FTData.size()>0&&tempFTD.size()>0)
			{
				for(int i=0; i<FTData.size();i++)
				{
			...
			if(matches.size()>0)
			{
				...
				for(int h=0; h<FTData.size();h++)
				{
					FTData[h].associate=false;
				}

				for(int m=0;matchesA.size()>m;m++)
				{
					if(matchesA[m].imgIdx==1)
						FTData[matchesA[m].trainIdx].associate=true;
				}	

				for(int h=0; h<FTData.size();h++)
				{
					if(FTData[h].associate==false)
						mergeFTD.push_back(FTData[h]);
				}

				for(int m=0;matches.size()>m;m++)
				{
					double dselalpha=0.0;
					double dDist=0.0;
					double dminDist=10000000.0;
					...
					trans_img2global((float)FTData[nIDXB].Rx, (float)FTData[nIDXB].Ry, (float)FTData[nIDXB].Rt,
						FTData[nIDXB].u, FTData[nIDXB].v, fDFx, fDFy, fDFz,
						KuSiriusCameraInterface::getInstance()->getCamContext(),m_dHeight);
					...
					if(_hypot((double)(tempFTD[nIDXA].Rx-FTData[nIDXB].Rx),(double)(tempFTD[nIDXA].Ry-FTData[nIDXB].Ry))>0.1)
					{
						trans_global2img((float)FTData[nIDXB].Rx, (float)FTData[nIDXB].Ry, (float)FTData[nIDXB].Rt,
							tempFTD[nIDXA].x,  tempFTD[nIDXA].y, tempFTD[nIDXA].ceilingheight,nu,nv,
							KuSiriusCameraInterface::getInstance()->getCamContext());

						double dImageD=_hypot(nu-FTData[nIDXB].u,nv-FTData[nIDXB].v);
					...
					if(matches[m].distance<75)
					{
						for(double alpha=-2.4; alpha<10.0;alpha+=0.1)
						{
							...
							trans_img2global((float)FTData[nIDXB].Rx, (float)FTData[nIDXB].Ry, (float)FTData[nIDXB].Rt, 
								FTData[nIDXB].u, FTData[nIDXB].v, fDFx, fDFy, fDFz,KuSiriusCameraInterface::getInstance()->getCamContext(),m_dHeight+alpha);


							dDist=_hypot(fFx-fDFx,fFy-fDFy);
						...
						{
							tempFTD[nIDXA].ceilingheight=(tempFTD[nIDXA].ceilingheight*tempFTD[nIDXA].nmatchnum+m_dHeight+dselalpha)/(double)(tempFTD[nIDXA].nmatchnum+1);
							...
							tempFTD[nIDXA].x=fFx;
							tempFTD[nIDXA].y=fFy;
							tempFTD[nIDXA].th=(tempFTD[nIDXA].th*tempFTD[nIDXA].nmatchnum+FTData[nIDXB].th)/(double)(tempFTD[nIDXA].nmatchnum+1);	
							tempFTD[nIDXA].nmatchnum=tempFTD[nIDXA].nmatchnum+1;
							
							keypoints.angle = FTData[nIDXB].angle;
							keypoints.response = FTData[nIDXB].response;
							keypoints.pt.x = FTData[nIDXB].u;
							keypoints.pt.y = FTData[nIDXB].v;
							keypoints.size = 7.0;
							...
							line(PImage, Point(FTData[nIDXB].u, FTData[nIDXB].v), Point(tempFTD[nIDXA].u, tempFTD[nIDXA].v), Scalar(0, 0, nColor), 2, 8);
			...
			else
			{
				...
				if(FTData.size()>0)
				{
					for(int k=0;k<FTData.size();k++)
					{
						mergeFTD.push_back(FTData[k]);
					}
				}
			}	
...
// 			cvWaitKey(10);
			...
			for(int i=0; i<mergeFTD.size();i++)
			{
				for(int j=0; j<i;j++)
				{	
					double dDist= hypot(mergeFTD[i].x-mergeFTD[j].x,mergeFTD[i].y-mergeFTD[j].y);
					if(dDist<0.3)
					{			
	...
	else
	{
		...
		vector<FRegionData> FRD;vector<FTampletData> FTD;
		m_ParticleFilter.getFeature(&FRD,&FTD);
		KuDrawingInfo::getInstance()->setRegion(FRD);
		KuDrawingInfo::getInstance()->setTampletData(FTD);
...
void KuILBPFLocalizerPr::saveFeatureData()
{
	...
	for(i=0; i<m_FRD.size() ; i++){
		if(m_FRD[i].add==true&&m_FRD[i].nmatchnum>10)
		{
			m_FRD[i].angle=0.0;
...
/**
 ...
 @brief English: 
*/
vector<FRegionData> KuILBPFLocalizerPr::loadFeatureData( )
{
	char cFilePathName[150];
	memset(cFilePathName,0,sizeof(cFilePathName));
	sprintf_s(cFilePathName,"./Data/FeatureData/FeatureData.log");	
...
void KuILBPFLocalizerPr::saveTampleteData()
{
	char cFilePathName[150];
	memset(cFilePathName,0,sizeof(cFilePathName));
	sprintf_s(cFilePathName,"./Data/FeatureData/TampleteData.log");

	ofstream DataLog;
	DataLog.open(cFilePathName);
	int i=0;
	for(i=0; i<m_FTD.size() ; i++){
		if(m_FTD[i].add==true&&m_FTD[i].nmatchnum>50)
		{
			DataLog<<m_FTD[i].u<<" ";
			DataLog<<m_FTD[i].v<<" ";
			DataLog<<m_FTD[i].x<<" ";
...
/**
 ...
 @brief English: 
*/
vector<FTampletData> KuILBPFLocalizerPr::loadTampleteData( )
{
	char cFilePathName[150];
	memset(cFilePathName,0,sizeof(cFilePathName));
	sprintf_s(cFilePathName,"./Data/FeatureData/TampleteData.log");	
	...
	DataLog.close();
	m_ParticleFilter.setTamplet(m_FTD);
	return m_FTD;
}
...
vector<FRegionData>  KuILBPFLocalizerPr::getRegion()
{
	vector<FRegionData> FRD;
...
vector<FTampletData>  KuILBPFLocalizerPr::getTampletData()
{

	vector<FTampletData> FTD;
	m_CriticalSection.Lock();
	for(int i=0; i<m_FTD.size();i++)
	{
...
void KuILBPFLocalizerPr::copyCeilingImage(IplImage * IplCeilingImage)
{
	...
	IplEqualizeCeilingImage = cvCreateImage(cvSize(320,240),8,1);
