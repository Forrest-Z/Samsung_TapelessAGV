...
void KuILBPFLocalizerPr::eliminateFREAKData(KuPose RobotPos,vector<CFREAKData>*mergeFTD)
{
	...
	for(int i=0; i<(*mergeFTD).size();i++)
	{
		for(int j=0; j<i;j++)
		{	
			double dDist= _hypot((*mergeFTD)[i].x-(*mergeFTD)[i].x,(*mergeFTD)[i].y-(*mergeFTD)[j].y);
			if(dDist<0.3)
			{			
...
/**
 ...
 @brief English: 
*/
vector<CFREAKData> KuILBPFLocalizerPr::loadTampleteData( )
{
	char cFilePathName[150];
	memset(cFilePathName,0,sizeof(cFilePathName));
	sprintf_s(cFilePathName,"./Data/FeatureData/TampleteData.log");	
...
vector<CFREAKData>  KuILBPFLocalizerPr::getTampletData()
{
