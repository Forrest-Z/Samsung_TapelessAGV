	...
	{		
		...
		for(int j=0; j<i;j++)
		{
			...
			{			
				...
				double dTheta1=(*FRD)[j].th-90;
				double dTheta2=(*FRD)[i].th-90;
				printf("dTheta1=%f,dTheta2=%f\n",dTheta1,dTheta2);
...
void KuILBPFLocalizerPr::estimateRobotPosUsingParticleFilter()
{
	...
	double dLampNM=0;
	int nLampNUM=0;
	double dLampillmuination=/*KuRobotParameter::getInstance()->getLampillmuination( )*255;*/225;
	...
	if(m_bMapping==true)
	{
		...
		for(int i = 0; i < Sensor::CEILING_IMAGE_WIDTH * Sensor::CEILING_IMAGE_HEIGHT; i++)
		{
			...
			if(m_cvMatImage.data[i]<dLampillmuination){
