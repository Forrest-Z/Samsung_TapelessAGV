...
{
	m_cvMatResultImage.create(240,320,CV_8UC1);//Result image initialization
...
bool KuSiriusCameraInterface::initCam(CWnd* pWnd, ImageFormat format, bool bViewSettingsWindow /*= false*/)
{
	...
	*m_hWndDisp = capCreateCaptureWindow(_T("Camera"),
	...
	//	capPreviewRate(*m_hWndDisp, 30);
	//	capOverlay(*m_hWndDisp, false);
	//	capPreview(*m_hWndDisp, false);
...
/**
 @brief Korean: 카메라와 connection을 수행한다.
 @brief English: Image undistortion function
*/
bool KuSiriusCameraInterface::connect() 
{
	m_cvVideoCap.open(-1);
	if( !m_cvVideoCap.isOpened() ){
		cout << "Could not find camera resource!!!"<<endl;
		bSiriusCameraflag=false;
		return false;
	}
	else
	{
		bSiriusCameraflag=true;
		m_cvVideoCap.set(CV_CAP_PROP_FRAME_WIDTH,320);
		m_cvVideoCap.set(CV_CAP_PROP_FRAME_HEIGHT,240);
		return true;
	}
...
/**
 @brief Korean: 왜곡 보정된 이미지 정보를 넘겨준다.
 @brief English: Image undistortion function
*/
Mat KuSiriusCameraInterface::getData()
{
	if(bSiriusCameraflag)
	{
		m_cvVideoCap.set(CV_CAP_PROP_AUTO_EXPOSURE);
		//m_cvVideoCap >> m_cvMatImage;
		if(m_cvVideoCap.read(m_cvMatImage) != NULL)
		{
			calcUndistortedImage();//Image undistortion
		}
	}

	return m_cvMatResultImage;
}

/**
 @brief Korean: 왜곡 보정 함수 
 @brief English: Image undistortion function
*/
// void KuSiriusCameraInterface::calcUndistortedImage(void)
// {
// 	int nImg_width = Sensor::CEILING_IMAGE_WIDTH;
// 	int nImg_height = Sensor::CEILING_IMAGE_HEIGHT;
// 
// 	double dCam_fx = m_dCam_fx;
// 	double dCam_fy = m_dCam_fy;
// 	double dCam_cx = m_dCam_cx;
// 	double dCam_cy = m_dCam_cy;
// 	double dCam_d1 = m_dCam_d1;
// 	double dCam_d2 = m_dCam_d2;
// 	double dCam_d3 = m_dCam_d3;
// 	double dCam_d4 = m_dCam_d4;
// 
// 	//Camera and distortion parameter
// 	Mat MatImage;
// 	MatImage.create(nImg_height,nImg_width,CV_8UC1);
// 	cvtColor(m_cvMatImage,MatImage,CV_RGB2GRAY);
// 
// 	Mat matCameraMatrix=(Mat_<double>(3,3)<<dCam_fx,0.0,dCam_cx,0.0,dCam_fy,dCam_cy,0.0,0.0,1.0);
// 	Mat matDistortionMatrix=(Mat_<double>(4,1)<<dCam_d1,dCam_d2,dCam_d3,dCam_d4);
// 
// 	undistort(MatImage,m_cvMatResultImage,matCameraMatrix,matDistortionMatrix);
// 	
// 	MatImage.release();
// 	matCameraMatrix.release();
// 	matDistortionMatrix.release();
// 
// }
