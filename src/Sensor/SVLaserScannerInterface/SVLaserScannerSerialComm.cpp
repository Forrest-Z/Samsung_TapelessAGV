...
IMPLEMENT_DYNCREATE(SVLaserScannerSerialComm, CObject)

SVLaserScannerSerialComm::SVLaserScannerSerialComm()
{
...
void SVLaserScannerSerialComm::init()
{
	hComm=NULL;				// Initialization the Handle for Comport
	bFlowCtrl=FC_XONXOFF;	// Setup the flow_control
	fConnected=FALSE;		// thread stop
...
int SVLaserScannerSerialComm::GetBlock(char* data)
{
	...
	{	
		...
		if(local_buf_rd_idx == SV_MAXBLOCK) local_buf_rd_idx=0;
	}
...
void SVLaserScannerSerialComm::PutData(char data)
{
	m_CriticalSection.Lock(); 

	local_buff[local_buf_wr_idx++]=data;
	if(local_buf_wr_idx == SV_MAXBLOCK) local_buf_wr_idx=0;
...
char SVLaserScannerSerialComm::GetData(void)
{
	bNotAvail=FALSE;
	if(local_buf_rd_idx == local_buf_wr_idx) 
	{
...
void SVLaserScannerSerialComm::SetReadData(char* data, int len)
{
	m_CriticalSection.Lock(); 
...
void SVLaserScannerSerialComm::SetHwnd(HWND hwnd)
{
	m_hWnd=hwnd;
}

// set up comport
void SVLaserScannerSerialComm::SetComport(int port, DWORD rate, BYTE byteSize, BYTE stop, BYTE parity)
{
	bPort=port;
	dwBaudRate=rate;
	bByteSize=byteSize;
...
void SVLaserScannerSerialComm::SetDtrRts(BYTE chk)
{
	bFlowCtrl=chk;
}
...
BOOL SVLaserScannerSerialComm::CreateCommInfo()
{
	osWrite.Offset=0;
...
bool SVLaserScannerSerialComm::OpenComport()
{
	char szPort[15];
	BOOL fRetVal;
...
BOOL SVLaserScannerSerialComm::SetupConnection()
{
	BOOL fRetVal;
	//BYTE bSet;
	DCB dcb;
...
BOOL SVLaserScannerSerialComm::DestroyComm()
{
...
// close connection
BOOL SVLaserScannerSerialComm::CloseConnection()
{
...
*/
void SVLaserScannerSerialComm::sendData(char* pSendData, int nDataLength)
{
...
BOOL SVLaserScannerSerialComm::WriteCommBlock(LPSTR lpByte, DWORD dwBytesToWrite)
{
	BOOL fWriteStat;
...
/**
 ...
 @brief English: write in English
 */
int SVLaserScannerSerialComm::readData(unsigned char* pRecvdData, int nDataLength)
{
	int nLength=0;	
	unsigned char InData[SV_MAXBLOCK+1];
...
int SVLaserScannerSerialComm::ReadCommBlock(LPSTR lpszBlock, int nMaxLength)
{
	BOOL fReadStat;
	COMSTAT ComStat;
	DWORD dwErrorFlags;
